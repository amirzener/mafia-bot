...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith(("join_player:", "join_observer:", "start_game:")):
...
async def handle_game_actions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "join_player":
        ...
        if user_id not in [p['id'] for p in list_data["players"]]:
            list_data["players"].append({"id": user_id, "name": username})
...
async def update_active_list_message(list_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    creator_name = "Admin"
    if is_owner(list_data["creator"]):
        creator_name = "Owner"
    else:
        admins = load_json(ADMIN_FILE)
        admin_info = admins.get(str(list_data["creator"]), {})
        if admin_info:
