        ...
        for channel_id in channels:      
            try:      
                keyboard = [      
                    ...
                    [InlineKeyboardButton("🚀 شروع بازی", callback_data=f"start_game:{list_id}")],  
                ]      
...
if query.data in ["show_groups", "show_channels", "show_admins", "back_to_main", "close_panel"]:  
...
if action == "join_player":  
    ...
    if user_id not in [p["id"] for p in list_data["players"]]:  
    ...
    if user_id not in [o["id"] for o in list_data["observers"]]:      
        list_data["observers"].append({"id": user_id, "name": user_name})      
        active_list[list_id] = list_data      
        save_json(ACTIVE_LIST_FILE, active_list)      
    ...
    for group_id in groups:
        try:
            ...
            for i in range(0, len(players), 5):
                batch = players[i:i+5]
                mentions = " ".join(
                    f"<a href='tg://user?id={p['id']}'>@{p.get('username', p['name'])}</a>"
                    if p.get('username')
                    else f"<a href='tg://user?id={p['id']}'>{p['name']}</a>"
                    for p in batch
                )
                await context.bot.send_message(
                    chat_id=group_id,
...
# آماده‌سازی اطلاعات سازنده  
creator_name = "مدیر"  
admins = load_json(ADMIN_FILE)  
if str(list_data["creator"]) in admins:  
    creator_name = admins[str(list_data["creator"])].get("alias", "مدیر")  

# آماده‌سازی لیست بازیکنان  
players_text = ""  
if list_data["players"]:  
    players_text = "\n".join(  
        f"{i+1}) [{p['name']}](tg://user?id={p['id']})"  
...
if list_data["observers"]:  
    observers_text = "\n".join(  
        f"👁️ [{o['name']}](tg://user?id={o['id']})"  
        for o in list_data["observers"])  
else:  
    observers_text = "هنوز ناظری ثبت نام نکرده است."  

# نمایش ساعت به فرمت 19:30  
game_time = list_data["time"]  
if len(game_time) == 4:  
    game_time = f"{game_time[:2]}:{game_time[2:]}"  
